...
export default function Login({ hTheme }) {
	...
	const login = (event) => {
		...
		signInWithEmailAndPassword(auth, userEmail, password)
			.then(res => {
				...
				localStorage.setItem("loginType", "emailPassword")
				nav("/");
			})
			.catch(err => {
				console.log(err);
		...
		<>
			<NavBar hTheme={hTheme}/>
				...
				<div className="relative flex flex-col justify-center min-h-[80vh] overflow-hidden">
					<div className="w-full p-6 m-auto mt-16 md:mt-20 bg-white dark:bg-black rounded-md shadow-xl sm:max-w-md">
						...
						<form className="mt-6">
							...
							<input
								...
								className="block w-full px-4 py-2 mt-2 text-purple-700 bg-white border rounded-md focus:border-purple-400 
									...
									dark:bg-black dark:text-purple-300"
								onChange={hUserEmail}
							/>
							</div>
							<div className="mb-2">
						...
						>
							...
							<div className="mt-6">
								<button className="w-full px-4 py-2 tracking-wide text-white transition-colors duration-200 transform bg-purple-700 rounded-md hover:bg-purple-600 focus:outline-none focus:bg-purple-600
									dark:bg-purple-300 dark:text-black"
									onClick={login}
								>
								Login
								</button>
						...
						<div className="relative flex items-center justify-center w-full mt-6 border border-t">
							<div className="absolute px-5 bg-white dark:bg-black">Or</div>
						</div>
